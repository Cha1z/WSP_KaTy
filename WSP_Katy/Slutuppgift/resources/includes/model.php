<?php
require 'resources/includes/db_conn.php'; // Kommentar här, Den säger att den behöver en uppkoppling från phpmyadmin

if ($pdo) {

    // Kommentar här, Visar inlägg och vem som skapade dem
    ...
    // Kommentar här, Sökrutan
    if (isset($_POST['search'])) {
        // Kommentar här, Visar vad man sökt efter
        $data = $_POST['what'];
        ...
        // Kommentar här, Visar skapade inlägg men bara om det finns något skrivet
        if (!empty($data)) {
    ...
    // Kommentar här, Skapar array
    $model = array();
    foreach($pdo->query($sql) as $row) {
        // Kommentar här, sorterar till row
        $model += array(
            $row['ID'] => array(
                'slug' => $row['Slug'],
                'title' => $row['Headline'],
