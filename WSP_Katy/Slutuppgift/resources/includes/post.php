...
if ($pdo) {
    ...
    // Kommentar här Skapar lista med ID och namn som array
    ...
    // Kommentar här, Grundkommando för variablerna
    if (isset($_POST['submit'])) {
        $user = $_POST['author']; //Kommentar här, vem som skapat ett inlägg
        $headline = $_POST['title']; //Kommentar här, Namnet på inlägget
        $headline = trim($headline);

        $slug = slugify($headline); //Kommentar här, gör meningar lättare att läsa och så det inte går att förändras

        $text = $_POST['message']; //Kommentar här, visar ett helt inlägg
